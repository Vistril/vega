...
//Credit to Angush, GitHub
//https://github.com/Angush/trellobot
//Modified and updated to Discord.js v12 Standard for VEGA
...
Main.on('ready', () => {
    ...
    */
    ...
    if (!conf.prefix) {
        conf.prefix = "<@!717829830237749308> "
    ...
    console.log(`== Bot logged in as @${Main.user.tag}. Ready for action! ==`)
    events.start()
});
/*
** ====================================
...
const getEmbedBase = (event) => new Discord.MessageEmbed()
        .setFooter(`${conf.guild.members.cache.get(Main.user.id).displayName} • ${event.data.board.name} [${event.data.board.shortLink}]`, Main.user.displayAvatarURL)
